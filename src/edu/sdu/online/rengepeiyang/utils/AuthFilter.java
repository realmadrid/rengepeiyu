...
public class AuthFilter implements Filter {
 ...
 public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
  ...
  if (!"/preindex.jsp".equals(targetURL)&&!"/loadteacher.jsp".equals(targetURL)&&!"/loadclazz.jsp".equals(targetURL)) {//判断当前页是否是重定向以后的登录页面页面，如果是就不做session的判断，防止出现死循环
   if (session == null || session.getAttribute("user") == null) {//*用户登录以后需手动添加session
    response.sendRedirect(request.getContextPath() + "/preindex.jsp");//如果session为空表示用户没有登录就重定向到login.jsp页面
    return;
   }
  }
  if("/preindex.jsp".equals(targetURL) || "/".equals(targetURL)||"profile".equals(targetURL)) {
	  if (session != null && session.getAttribute("user") != null) {
		  response.sendRedirect("profile?student.sid="+((User)session.getAttribute("user")).getSid());
	  }
  }
  //加入filter链继续向下执行
